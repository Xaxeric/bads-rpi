...
class ImageCaptureClient:
    def __init__(
        self, api_url="http://192.168.18.11:3000/api/capture", capture_interval=0.5
    ):
    ...
    def capture_and_send_image(self):
        ...
        try:
            ...
            files = {"image": (filename, buffer.getvalue(), "image/jpeg")}
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✗ Failed to send image {self.capture_count}: HTTP {response.status_code}"
                )
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ["-h", "--help"]:
            print(
                "Usage: python3 capture_api_client.py [api_url] [interval] [duration]"
            )
            print(
                "  api_url:  REST API endpoint (default: http://192.168.18.11:3000/api/capture)"
            )
            ...
            print(
                "  python3 capture_api_client.py http://localhost:3000/api/capture 1.0"
            )
            print(
                "  python3 capture_api_client.py http://localhost:3000/api/capture 0.5 30"
            )
            sys.exit(0)
